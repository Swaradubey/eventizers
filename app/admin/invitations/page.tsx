...
function InvitationThumbnail({ src, alt }: { src?: string; alt: string }) {
  ...
  if (!src || hasError) {
    return (
      <div className="w-12 h-12 rounded-xl bg-blue-50/80 border border-blue-100 flex items-center justify-center shrink-0 shadow-2xs" title="No Preview Image">
  ...
  return (
    <div className="w-12 h-12 rounded-xl overflow-hidden border border-blue-100 shrink-0 bg-neutral-100 shadow-2xs">
      <img
        src={getImageUrl(src)}
        alt={alt}
        ...
        <div className="bg-white/90 border border-blue-100 rounded-2xl p-6 shadow-sm flex flex-col min-h-[500px] backdrop-blur-md">
          ...
          {loading ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center py-20">
              ...
              <h4 className="text-lg font-semibold text-slate-900">Error loading invitations</h4>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-blue-50">
                  {paginatedInvitations.map((inv) => (
                    <tr key={inv.id} className="hover:bg-blue-50/40 transition-colors duration-150 group">
                      ...
                      <td className="py-4 px-4 text-xs text-slate-700">
                        <div className="flex flex-col">
                          ...
                          <span className="text-[10px] text-slate-400">{inv.eventCreator?.email || "-"}</span>
                        </div>
                      </td>
        ...
        {viewingInvitation && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 md:p-6">
            ...
            <motion.div
              ...
              className="relative bg-white w-full max-w-lg max-h-[90vh] rounded-2xl shadow-2xl border border-blue-100 overflow-hidden z-10 flex flex-col text-slate-800 font-body"
              ...
              <div className="overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-5 custom-scrollbar" style={{ padding: "24px" }}>
                ...
                <div className="space-y-4 text-xs font-semibold text-slate-600 uppercase tracking-wider">
                  {viewingInvitation.imageUrl && (
                    <div className="w-full h-36 sm:h-48 rounded-xl overflow-hidden border border-blue-100 bg-neutral-100 shadow-sm relative">
                      <img
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-slate-900 mt-0.5 normal-case break-words">{viewingInvitation.title}</p>
                  </div>
                  {viewingInvitation.subtitle && (
                    <div>
                      <span className="text-[10px] text-slate-400">Subtitle</span>
                      <p className="text-sm font-semibold text-slate-800 mt-0.5 normal-case break-words">{viewingInvitation.subtitle}</p>
                    </div>
                  )}
                  {viewingInvitation.mainText && (
                    <div>
                      <span className="text-[10px] text-slate-400">Custom Message</span>
                      <p className="text-xs font-medium text-slate-700 mt-0.5 normal-case whitespace-pre-line leading-relaxed bg-blue-50/40 p-3 rounded-xl border border-blue-100/80 break-words">{viewingInvitation.mainText}</p>
                    </div>
                  )}
                  <div>
                    <span className="text-[10px] text-slate-400">Event Title</span>
                    <p className="text-xs font-bold text-blue-600 mt-0.5 normal-case break-words">{viewingInvitation.eventTitle}</p>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400">Event Creator</span>
                    <p className="text-xs font-semibold text-slate-700 mt-0.5 normal-case break-words">
                      {viewingInvitation.eventCreator?.name || "-"} ({viewingInvitation.eventCreator?.email || "-"})
                    </p>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400">Status</span>
                    <p className="text-xs font-bold text-blue-700 mt-0.5 uppercase">{viewingInvitation.status}</p>
                  </div>
                </div>
                ...
                <div className="flex justify-end pt-3 border-t border-blue-100">
                  <button
                    onClick={() => setViewingInvitation(null)}
                    className="w-full sm:w-auto px-5 py-2.5 sm:py-2 text-xs font-semibold text-white bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 rounded-xl active:scale-95 transition-all shadow-md shadow-blue-500/20 focus:outline-none focus:ring-2 focus:ring-blue-500/40"
                  >
                    Close
                  </button>
                </div>
        ...
        {isEditModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4">
            ...
            <motion.div
              ...
              exit={{ opacity: 0, scale: 0.95 }}
              onClick={(e) => e.stopPropagation()}
              className="relative bg-white w-full max-w-md max-h-[90vh] rounded-2xl shadow-2xl border border-blue-100 overflow-hidden z-10 flex flex-col text-slate-800 font-body"
              style={{ maxHeight: "calc(100vh - 40px)" }}
            >
