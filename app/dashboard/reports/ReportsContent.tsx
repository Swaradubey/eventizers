...
export function ReportsContent() {
  ...
  const loadReportData = useCallback(async (eventId: string) => {
    setLoading(true);
    setError(null);
    try {
      ...
      if (summaryRes && summaryRes.success) {
        ...
        setSummary({ checkedIn: 0, pending: 0, total: 0 });
      }
  ...
  }, [selectedEventId, loadReportData]);
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-blue-50 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-normal text-slate-800 force-georgia">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
          ...
          <div className="flex items-center justify-between mt-3 text-xs text-slate-600">
            <div className="flex items-center gap-2">
              ...
              <span>
                Checked In ({total > 0 ? Math.round((checkedIn / total) * 100) : 0}%)
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-amber-400 inline-block" />
              <span>
                Pending Arrival ({total > 0 ? Math.round((pending / total) * 100) : 0}%)
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm">
          <div className="p-5 border-b border-slate-100 flex items-center justify-between">
            ...
            <button
              onClick={() =>
                router.push(
                  `/dashboard/check-in${selectedEventId ? `?eventId=${selectedEventId}` : ""}`
                )
              }
