...
function GpsCheckInContent() {
  ...
  useEffect(() => {
    async function loadEvents() {
      ...
      try {
        ...
        if (list.length > 0) {
          // If query parameter event exists and is in list, select it; otherwise find "Community Meetup" or first event
  ...
  const fetchLocation = (showToast = false) => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        // Calculate distance if current event has coordinates
  ...
  const handleGpsCheckIn = async () => {
    ...
    // Prompt fresh browser geolocation
  ...
  // Helper: Synchronize venue location to current position (Convenience feature for instant testing)
    ...
    <div className="flex-1 flex flex-col min-h-screen bg-slate-50/50">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 sm:px-6 py-6 md:py-8 space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 p-6 sm:p-8 text-center shadow-sm relative overflow-hidden">
          ...
          <h1 className="text-xl sm:text-2xl font-normal text-slate-900 tracking-tight leading-snug force-georgia">
